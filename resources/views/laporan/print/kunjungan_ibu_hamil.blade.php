          ...
          <tbody>
            <?php foreach($laporan as $key => $value) { ?>
              <tr>
                ...
                $riwayat_kehamilan = DB::table('riwayat_kehamilan')->where('id_layanan_ibu_hamil',$value->id)->get();
                ...
                <td>{{$riwayat_kehamilan[0]->kehamilan_ke}}</td>
                <td>{{date('d-m-Y', strtotime($value->hpht))}}</td>
                <td>{{$value->lila}}</td>
                <td>{{$value->hasil_skor_kspr}}</td>
                <td>{{$value->g.' '.datediff('ww', date('d-m-Y', strtotime($tgl_awal)), date('d-m-Y', strtotime($value->hpht)), false).' minggu'}}</td>
              </tr>
            <?php } ?>
          </tbody>
        </tbody>
